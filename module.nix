...
in {
  options.services.joplin-server = {
    ...
    storage = {
      driver = mkOption {
        type = types.enum [ "Database" "Filesystem" "S3" ];
        default = "Database";
    ...
    mailer = {
      ...
      security = mkOption {
        type = types.enum [ "tls" "starttls" "none" ];
        default = "starttls";
  ...
  config = mkIf cfg.enable {
    ...
    systemd.services.joplin-server = mkIf (!cfg.useContainer) {
      ...
      after = [ "network.target" ] ++ optional (cfg.database.type == "postgres" && cfg.database.createLocally) "postgresql.service";
      wants = optional (cfg.database.type == "postgres" && cfg.database.createLocally) "postgresql.service";
      ...
      serviceConfig = {
        ...
        User = cfg.user;
        Group = cfg.group;
        ExecStart = if cfg.package != null then "${cfg.package}/bin/joplin-server" else "/run/current-system/sw/bin/joplin-server";
        Restart = "on-failure";
        RestartSec = "10s";
    ...
    virtualisation.oci-containers.containers.joplin-server = mkIf cfg.useContainer {
      ...
      volumes = optional (cfg.storage.driver == "Filesystem") "${cfg.storage.path}:/var/lib/joplin-server";
    };
